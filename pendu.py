...
with open("liste_de_mots.txt") as f: #ouvre la liste de mots
    lines = f.readlines() 
    motATrouver = random.choice(lines)[:-1] #prends une ligne aleatoire dans le mot 
...
while jeuEnCours:
    ...
    if lettreDansLaListe == False : #si la lettre n'est pas déjà dans la liste, ça la met dedans
        listeLettres += lettre
